...
// import { PlainFooter } from "@uniwebcms/express";

// export default (props) => {
//   const profile = props.page.getPageProfile();
//   console.log(profile, profile.getBasicInfo());
//   return <PlainFooter {...props} profile={profile} />;
// };
...
import { Container } from "@uniwebcms/express";
...
export default function Plain(props) {
  ...
  const title = "profile.getBasicInfo()";
  // console.log(page.getPageProfile());
  ...
  return (
    <div className={twJoin(theme, as === "footer" && "mt-32")}>{markup}</div>
  );
}
